...
MODEL_COMMANDS = ("train", "evaluate", "export_models", "inspect_checkpoint")
...
def execute_model_command(command: str, argument: str) -> int:
    ...
    try:
        ...
        if isinstance(result, EvaluationReport) and not result.completed:
            logger.info("experimental_model_evaluation_incomplete")
            return 1
        logger.info("experimental_model_command_completed")
        if isinstance(result, TrainingRun) and result.stop_signal is not None:
